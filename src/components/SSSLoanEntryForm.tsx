...
export interface SSSLoanEntryFormProps {
  /** Loan type discriminator persisted in storage */
  loanType: Loan['loanType'];
  /** Human readable module label, e.g. "SSS Emergency Loan" */
  label: string;
  /** Prefix used when generating loan ids */
  idPrefix: string;
  /** Tailwind classes for the accent color of the module */
  accent: {
    text: string;
    bg: string;
    button: string;
...
/**
 * Shared entry form used by both the SSS Emergency Loan and SSS Calamity Loan
 * modules. All CRUD, reloan and scheduler behaviour flows through the existing
 * storage layer so a future SQLite swap only requires changing `utils/storage`.
 */
const SSSLoanEntryForm: React.FC<SSSLoanEntryFormProps> = ({ loanType, label, idPrefix, accent }) => {
  ...
  const handleSearchEmployee = () => {
    ...
    if (matches.length === 0) {
      toast({
        ...
        description: `No active ${label}s found for ${searchEmployee}.`,
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      title: 'Loan Loaded',
      description: `Loaded ${label} for ${latest.employeeName}.`,
    });
  };
  ...
  const handleDelete = () => {
    if (!formData.employeeName.trim()) {
      toast({
        ...
        description: `Please enter an employee name to delete their ${label}s.`,
        variant: 'destructive',
      });
      return;
    }
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${label} data for ${formData.employeeName}? This will only delete unpaid payments, paid payments will remain.`
      )
    )
      return;
    ...
    if (targetLoans.length === 0) {
      toast({
        title: 'No Loans Found',
        description: `No ${label}s found for ${formData.employeeName}.`,
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      title: `${label}s Deleted`,
      description: `Deleted ${targetLoans.length} ${label}(s) for ${formData.employeeName}. Paid payments preserved.`,
    });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Only one active loan of this type per employee
    if (!isEditing && !formData.isReloan) {
      ...
      if (hasActive) {
        toast({
          ...
          description: `${formData.employeeName} already has an active ${label}. Mark as Reloan to replace it.`,
          variant: 'destructive',
        });
        return;
      }
    ...
    if (isEditing && editingLoanId) {
      ...
      // Reloan handling — deactivate existing active loans of this type for this employee
  ...
  return (
    <div className="min-h-screen p-4">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <Button onClick={() => navigate('/dashboard')} variant="outline" className="mb-4 glass-panel">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Dashboard
          </Button>
          <h1 className="text-3xl font-bold text-3d">{label} Application</h1>
          <p className="text-muted-foreground">
            {isEditing ? 'Edit existing' : 'Create a new'} {label}
          </p>
        ...
        <Card className="glass-panel">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <div className={`icon-3d w-8 h-8 rounded-lg flex items-center justify-center ${accent.bg}`}>
                <AlertCircle className="h-5 w-5 text-white" />
              </div>
              <span>{isEditing ? `Edit ${label}` : `${label} Application`}</span>
            </CardTitle>
          </CardHeader>
          <CardContent>
...
export default SSSLoanEntryForm;
